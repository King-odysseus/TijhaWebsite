import { Link } from 'react-router-dom';

function Hero() {
  return (
    <section
      className="w-full pt-28 pb-16 md:pb-32 relative overflow-hidden bg-cover bg-center bg-no-repeat"
      style={{
        backgroundImage: `url('/images/Team.png')`,
      }}
    >
      {/* Dark overlay */}
      <div className="absolute inset-0 bg-indigo/75" />

      {/* Subtle gradient overlay */}
      <div className="absolute inset-0 bg-gradient-to-b from-indigo/40 via-transparent to-indigo/60" />

      <div className="w-full px-6 md:px-12 lg:px-20 relative z-10 flex flex-col items-center text-center">
        {/* Headline */}
        <h1 className="text-3xl md:text-4xl lg:text-6xl text-center max-w-4xl mx-auto text-white leading-[1.15] font-bold tracking-tight mt-[40px] mb-[20px]">
          Your Partner In Digital And Operational Excellence
        </h1>

        {/* Subtitle */}
        <p className="text-base md:text-lg text-white/70 text-center max-w-2xl mx-auto mb-10 leading-relaxed">
          At Tijha, we bridge strategy with execution to transform how organizations operate and grow. Through data-driven insight and digital innovation, we deliver measurable impact and sustainable performance.
        </p>

        {/* Trusted by */}
        <div className="flex flex-wrap items-center justify-center gap-4 mb-10">
          <div className="flex items-center gap-2 text-white/80 text-sm font-medium">
            <svg className="w-5 h-5 text-bronze" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm0 4.9c1.67 0 3.07 1.13 3.55 2.67l-1.16.39c-.33-.99-1.26-1.71-2.39-1.71s-2.06.72-2.39 1.71l-1.16-.39C8.93 7.03 10.33 5.9 12 5.9zM12 19c-3.86-1.14-6.83-4.58-7.15-8.9h1.52c.29 3.22 2.53 5.93 5.63 6.78V19zm0-2.23V10h1.5v3.41l2.33 2.33-1.06 1.06L12 14.65z" />
            </svg>
            <span>Trusted by</span>
          </div>
          <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2">
            <span className="text-base md:text-lg font-bold text-white">SMEs</span>
            <span className="text-base md:text-lg font-bold text-white">NGOs</span>
            <span className="text-base md:text-lg font-bold text-white">Corporates</span>
            <span className="text-base md:text-lg font-bold text-white">Government</span>
            <span className="text-base md:text-lg font-bold text-white">Startups</span>
          </div>
        </div>

        {/* CTA Button */}
        <Link
          to="/services"
          className="bg-bronze hover:bg-[#b8895c] text-indigo text-sm md:text-base px-8 py-3.5 rounded-full transition shadow-lg hover:shadow-xl hover:-translate-y-0.5 font-semibold"
        >
          See Services
        </Link>
      </div>
    </section>
  );
}

export default Hero;
